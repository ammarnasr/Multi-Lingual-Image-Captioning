...
def load_model(args, src_lang):
    ...
    model = ClipCaptionPrefix(args.prefix_length, lang=src_lang, clip_length=args.clip_length, prefix_size=args.prefix_size, num_layers=args.num_layers)
...
def train(dataset, model, args , start_epoch = 0):
    ...
    for epoch in range(start_epoch, epochs+start_epoch):
        ...
        for idx, (tokens_arabic, mask_arabic, tokens_english, mask_english, prefix) in enumerate(train_dataloader):
...
def main(args):
    biling_data_path = 'data/embeddings/multilingual_CLIP-ViT-B-32_embeddings.pkl'
    biling_lang_src = 'english'
    biling_lang_tgt = 'arabic'
    ck = args.checkpoint
    if ck is None:
        start_epoch = 0
        model = ClipCaptionPrefix(args.prefix_length, lang=biling_lang_src, clip_length=args.clip_length, prefix_size=args.prefix_size, num_layers=args.num_layers)
    else:
        model, args, start_epoch = load_model(args,biling_lang_src)
